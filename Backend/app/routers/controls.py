...
from ..models import Control, EvidenceRequest, User, Project
...
def create_control(payload: ControlCreate, db: Session = Depends(get_db), me: User = Depends(get_current_user)):
    ...
    if payload.project_id and not db.query(Project).filter(Project.id == payload.project_id).first():
...
def list_controls(
    ...
    q: Optional[str] = Query(None, description="search by tag or name"),
    db: Session = Depends(get_db),
    me: User = Depends(get_current_user)
):
    query = db.query(Control).options(joinedload(Control.owner)).order_by(Control.audit_year.desc(), Control.created_at.desc())
    if project_id:
        query = query.filter(Control.project_id == project_id)
    items = query.all()
    outs = []
    for c in items:
        if q and (q.lower() not in c.control_id_tag.lower() and q.lower() not in (c.name or "").lower()):
            continue
...
def get_control(control_id: int, db: Session = Depends(get_db), me: User = Depends(get_current_user)):
    ...
    d = ControlOut.model_validate(c).model_dump()
    if me.role == "Client" and not c.released_to_client: d["final_report_text"] = None
    return d
